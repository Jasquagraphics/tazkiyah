...
export default function ArtistsManagement() {
  ...
  const [artistsView, setArtistsView] = useState<'list' | 'grid'>('list');
  const [showRevenueModal, setShowRevenueModal] = useState(false);
  const [revenueData, setRevenueData] = useState({ amount: '', platform: 'Spotify', releaseId: 0 });
  const [successMessage, setSuccessMessage] = useState('');
  const [showAgreementModal, setShowAgreementModal] = useState(false);
  const [agreementArtist, setAgreementArtist] = useState<User | null>(null);
  ...
  const [agreementShares, setAgreementShares] = useState({ artistShare: 50, labelShare: 50 });
  ...
  const fetchAgreement = async (artistId: number) => {
    setAgreementLoading(true);
    setAgreementError('');
    try {
      const res = await fetch(`/api/admin/artist/${artistId}/agreement`);
      const data = await res.json();
      if (!res.ok) {
        setAgreementError(data?.error || 'Failed to load agreement');
        setAgreementData(null);
        return;
      }
      setAgreementData(data);
      if (typeof data?.artist_share === 'number' && typeof data?.label_share === 'number') {
        setAgreementShares({ artistShare: data.artist_share, labelShare: data.label_share });
      }
    } catch (err) {
  ...
  const openAgreement = async (artist: User) => {
    setAgreementArtist(artist);
    setShowAgreementModal(true);
    await fetchAgreement(artist.id);
  };

  const sendAgreement = async () => {
    if (!agreementArtist) return;
    const artist_share = Number(agreementShares.artistShare);
    const label_share = Number(agreementShares.labelShare);
    if (!Number.isFinite(artist_share) || !Number.isFinite(label_share) || artist_share < 0 || label_share < 0 || artist_share + label_share !== 100) {
      setAgreementError('Shares must be valid and total 100%.');
      return;
    }
    setAgreementSubmitting(true);
    setAgreementError('');
    try {
      const res = await fetch(`/api/admin/artist/${agreementArtist.id}/agreement`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ artist_share, label_share }),
      });
      const data = await res.json();
      if (!res.ok) {
        setAgreementError(data?.error || 'Failed to send agreement');
        return;
      }
      setSuccessMessage('Agreement sent successfully!');
      setTimeout(() => setSuccessMessage(''), 3000);
      await fetchAgreement(agreementArtist.id);
      await fetchArtists();
    } catch (err) {
      setAgreementError('Failed to send agreement');
    } finally {
  ...
  const verifyAgreement = async () => {
    ...
    setAgreementError('');
    try {
      const res = await fetch(`/api/admin/artist/${agreementArtist.id}/agreement/verify`, { method: 'POST' });
      const data = await res.json();
      if (!res.ok) {
        setAgreementError(data?.error || 'Failed to verify agreement');
        return;
      }
      setSuccessMessage('Agreement verified successfully!');
      setTimeout(() => setSuccessMessage(''), 3000);
      await fetchAgreement(agreementArtist.id);
      await fetchArtists();
    } catch (err) {
      setAgreementError('Failed to verify agreement');
    } finally {
      setAgreementSubmitting(false);
    }
  };

  const getAgreementButton = (artist: User) => {
    const s = artist.agreement_status;
    if (s === 'verified') return { label: 'Agreement Verified', disabled: true };
    if (s === 'signed') return { label: 'Verify Agreement', disabled: false };
    if (s === 'sent') return { label: 'Resend Agreement', disabled: false };
    return { label: 'Send Agreement', disabled: false };
  };

  const handleQuickAddRevenueForArtist = async (artistId: number) => {
    const data = await fetchArtistDetail(artistId);
    const releaseId = Number(data?.releases?.[0]?.id || 0);
    if (!releaseId) {
      setSuccessMessage('No releases found to attach revenue.');
      setTimeout(() => setSuccessMessage(''), 3000);
      return;
    }
    setRevenueData(prev => ({ ...prev, releaseId }));
  ...
  const handleDeleteArtist = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/admin/artist/${id}`, { method: 'DELETE' });
      if (res.ok) {
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                <div className="shrink-0 flex items-center gap-1 bg-white/5 p-1 rounded-2xl border border-white/5">
                  <button
                    type="button"
                    onClick={() => setArtistsView('list')}
                    className={cn(
                      "h-11 w-11 rounded-xl flex items-center justify-center transition-all",
                      artistsView === 'list' ? "bg-white/10 text-white" : "text-white/40 hover:text-white hover:bg-white/5"
                    )}
                    title="List view"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setArtistsView('grid')}
                    className={cn(
                      "h-11 w-11 rounded-xl flex items-center justify-center transition-all",
                      artistsView === 'grid' ? "bg-white/10 text-white" : "text-white/40 hover:text-white hover:bg-white/5"
                    )}
                    title="Grid view"
                  >
            ...
            {artistsView === 'grid' ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredArtists.map(artist => (
                  <div key={artist.id} className="glass-card p-6 md:p-8 group hover:border-brand-primary/50 transition-all duration-500">
                    <div className="flex items-center justify-between mb-8">
                      <div className="w-16 h-16 rounded-[1.5rem] vibrant-gradient p-[1px] shadow-xl shadow-brand-primary/20 overflow-hidden">
                        <div className="w-full h-full rounded-[23px] bg-[var(--app-bg-color,#030014)] flex items-center justify-center overflow-hidden">
                          <img 
                            src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${artist.name}&backgroundColor=transparent`} 
                    ...
                    <div className="flex items-center gap-3">
                      {artist.status === 'pending' && (
                        <button 
                          onClick={async () => {
                            const res = await fetch(`/api/admin/artist/${artist.id}/approve`, { method: 'PUT' });
                            if (res.ok) {
                              setArtists(artists.map(a => a.id === artist.id ? { ...a, status: 'approved' } : a));
                              setSuccessMessage('Artist approved successfully!');
                              setTimeout(() => setSuccessMessage(''), 3000);
                            }
                          }}
                          className="flex-1 bg-emerald-500/10 text-emerald-400 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-500/20 transition-all border border-emerald-500/20"
                        >
                          Approve Artist
                        </button>
                      )}
                      <button
                        type="button"
                        onClick={() => openAgreement(artist)}
                        disabled={getAgreementButton(artist).disabled}
                        className={cn(
                          "bg-white/5 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all border border-white/10",
                          artist.status === 'pending' ? "flex-none px-6" : "flex-1",
                          getAgreementButton(artist).disabled ? "opacity-60 cursor-not-allowed hover:bg-white/5" : ""
                        )}
                      >
                        {getAgreementButton(artist).label}
                      </button>
                      <button 
                        onClick={() => fetchArtistDetail(artist.id)}
                        className={cn(
                          "flex-1 bg-white/5 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all",
                          artist.status === 'pending' ? "flex-none px-6" : "flex-1"
                        )}
                      >
                        {artist.status === 'pending' ? <ChevronRight className="w-5 h-5" /> : 'Manage Artist'}
                      </button>
                      <button 
              ...
              <div className="space-y-4">
                {filteredArtists.map((artist) => (
                  <div key={artist.id} className="glass-card px-5 md:px-8 py-5 flex flex-col sm:flex-row sm:items-center gap-5 md:gap-6">
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                      <div className="w-12 h-12 rounded-2xl vibrant-gradient p-[1px] shrink-0 shadow-lg shadow-brand-primary/20 overflow-hidden">
                        <div className="w-full h-full rounded-[15px] bg-[var(--app-bg-color,#030014)] flex items-center justify-center overflow-hidden">
                          <img 
                            src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${artist.name}&backgroundColor=transparent`} 
                            className="w-full h-full object-cover" 
                            alt={artist.name}
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-3 min-w-0">
                          <p className="text-white font-black tracking-tight truncate">{artist.name}</p>
                          <div className="md:hidden">
                            <StatusBadge status={artist.status || 'pending'} />
                          </div>
                        </div>
                        <p className="text-[10px] font-black text-white/30 uppercase tracking-widest truncate">{artist.email}</p>
                      </div>
                    </div>
                    ...
                    <div className="hidden lg:block shrink-0 text-right">
                      <p className="text-[8px] font-black text-white/20 uppercase tracking-widest">Balance</p>
                      <p className="text-lg font-black text-white leading-none">${artist.balance?.toFixed(2) || '0.00'}</p>
                    </div>

                    <div className="flex items-center gap-2 flex-nowrap overflow-x-auto shrink-0 w-full sm:w-auto sm:ml-auto justify-start sm:justify-end md:max-w-[55%]">
                      {artist.status === 'pending' && (
                        <button 
                          type="button"
                          onClick={async () => {
                          ...
                          className="px-5 py-2.5 rounded-xl bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-all font-black text-[10px] uppercase tracking-widest whitespace-nowrap shrink-0 border border-emerald-500/20"
                        >
                          Approve
                        </button>
                      )}
                      <button
                        ...
                        onClick={() => openAgreement(artist)}
                        disabled={getAgreementButton(artist).disabled}
                        className="px-5 py-2.5 rounded-xl bg-white/5 text-white/60 hover:text-white hover:bg-white/10 transition-all font-black text-[10px] uppercase tracking-widest whitespace-nowrap shrink-0"
                      >
                        {getAgreementButton(artist).label}
                      </button>
                      <button
                        type="button"
                        onClick={() => fetchArtistDetail(artist.id)}
                        className="px-5 py-2.5 rounded-xl bg-brand-primary/10 text-brand-primary hover:bg-brand-primary/20 transition-all font-black text-[10px] uppercase tracking-widest whitespace-nowrap shrink-0"
                      >
                        Review
                      </button>
                      <button
                        type="button"
                        onClick={() => handleQuickAddRevenueForArtist(artist.id)}
                        className="p-3 rounded-xl bg-white/5 text-white/60 hover:text-white hover:bg-white/10 transition-all shrink-0"
                        title="Add revenue"
                      >
                      ...
                      <button 
                        type="button"
                        onClick={() => handleDeleteArtist(artist.id)}
                        className="p-3 rounded-xl bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 transition-all shrink-0"
                        title="Delete artist"
                      >
                        <Trash2 className="w-5 h-5" />
                      </button>
              ...
              <div className="lg:col-span-1 space-y-10">
                <div className="glass-card p-6 md:p-10">
                  <div className="flex flex-col items-center text-center">
                    <div className="w-24 h-24 md:w-32 md:h-32 rounded-[2rem] md:rounded-[2.5rem] vibrant-gradient p-[1px] shadow-2xl shadow-brand-primary/30 mb-8 overflow-hidden">
                      <div className="w-full h-full rounded-[31px] md:rounded-[39px] bg-[var(--app-bg-color,#030014)] flex items-center justify-center overflow-hidden">
                        <img 
                          src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${artistDetail.name}&backgroundColor=transparent`} 
                          className="w-full h-full object-cover" 
        ...
        {showAgreementModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              className="glass-card w-full max-w-5xl p-8 md:p-10 relative z-10"
            >
              <div className="flex items-start justify-between gap-6 mb-8">
                <div>
                  <h3 className="text-2xl md:text-3xl font-black text-white tracking-tight">Agreement</h3>
                  <p className="text-white/40 text-[10px] font-black uppercase tracking-widest mt-1">
                    {agreementArtist ? `${agreementArtist.name} • ${agreementArtist.email}` : 'Artist'}
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 text-white/50 hover:text-white hover:bg-white/10 transition-all flex items-center justify-center"
                  title="Close"
                >
                  <X className="w-5 h-5" />
                </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-1 space-y-6">
                    <div className="glass-card p-6 border border-white/10">
                      <p className="text-white/40 text-[10px] font-black uppercase tracking-widest mb-4">Revenue shares</p>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-black text-white/30 uppercase tracking-widest mb-2 ml-1">Artist (%)</label>
                          <input
                            type="number"
                            min={0}
                            max={100}
                            value={agreementShares.artistShare}
                            onChange={(e) => setAgreementShares({ ...agreementShares, artistShare: Number(e.target.value) })}
                            className="w-full px-4 py-3 rounded-2xl outline-none bg-white/5 border border-white/10 text-white"
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-black text-white/30 uppercase tracking-widest mb-2 ml-1">Label (%)</label>
                          <input
                            type="number"
                            min={0}
                            max={100}
                            value={agreementShares.labelShare}
                            onChange={(e) => setAgreementShares({ ...agreementShares, labelShare: Number(e.target.value) })}
                            className="w-full px-4 py-3 rounded-2xl outline-none bg-white/5 border border-white/10 text-white"
                          />
                        </div>
                      </div>
                      <div className="mt-4 flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
                        ...
                        <span className="text-white/70">{agreementData?.status || 'draft'}</span>
                      </div>
                    </div>

                    {agreementData?.signature_data_url ? (
                      <div className="glass-card p-6 border border-white/10">
                        <p className="text-white/40 text-[10px] font-black uppercase tracking-widest mb-4">Signature</p>
                        <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
                          <img src={agreementData.signature_data_url} alt="Signature" className="w-full h-20 object-contain" />
                        </div>
                      </div>
                    ) : null}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <button
                        ...
                        disabled={agreementSubmitting || !agreementArtist || agreementData?.status === 'verified'}
                        className="w-full py-4 rounded-2xl vibrant-gradient text-white font-black tracking-widest uppercase text-xs disabled:opacity-60"
                      >
                        {agreementData?.status === 'sent' ? 'Resend Agreement' : 'Send Agreement'}
                      </button>
                      <button
                        type="button"
                        onClick={verifyAgreement}
                        disabled={agreementSubmitting || !agreementArtist || !agreementData?.signature_data_url || agreementData?.status === 'verified'}
                        className="w-full py-4 rounded-2xl bg-white/5 border border-white/10 text-white/70 font-black tracking-widest uppercase text-xs hover:bg-white/10 transition-all disabled:opacity-60 disabled:hover:bg-white/5"
                      >
                        Verify
                      </button>
                    </div>
                  ...
                  <div className="lg:col-span-2 glass-card p-6 border border-white/10">
                    <div className="rounded-3xl border border-white/10 bg-white/5 overflow-hidden">
                      <div className="aspect-[16/10] bg-white/5">
                        {agreementData?.agreement_html ? (
                          <iframe title="agreement-preview" className="w-full h-full" sandbox="" srcDoc={agreementData.agreement_html} />
                        ) : (
