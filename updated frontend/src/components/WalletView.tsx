...
export default function WalletView({ user, onUpdate }: WalletViewProps) {
  ...
  const getStatements = () => {
    ...
    const grouped: Record<string, { id: string; title: string; period: string; amount: number; breakdown: any[] }> = {};
    ...
    history.forEach((item: any) => {
      ...
      if (statementFilter === 'monthly') {
        ...
        title = `Statement ${date.toLocaleDateString('default', { month: 'short', year: 'numeric' })}`;
        period = date.toLocaleString('default', { month: 'long', year: 'numeric' });
      } else if (statementFilter === 'yearly') {
        key = `${date.getFullYear()}`;
        title = `Annual Statement ${date.getFullYear()}`;
        period = `${date.getFullYear()}`;
      } else if (statementFilter === 'weekly') {
        // Simple week grouping
        const oneJan = new Date(date.getFullYear(), 0, 1);
        const numberOfDays = Math.floor((date.getTime() - oneJan.getTime()) / (24 * 60 * 60 * 1000));
        const week = Math.ceil((date.getDay() + 1 + numberOfDays) / 7);
        key = `${date.getFullYear()}-W${week}`;
        title = `Weekly Statement - Week ${week}`;
        period = `Week ${week}, ${date.getFullYear()}`;
      } else {
        key = item.date.split('T')[0];
        title = `Daily Statement ${key}`;
        period = key;
      }
      ...
      const platformIndex = grouped[key].breakdown.findIndex(b => b.platform === item.platform);
      if (platformIndex > -1) {
        grouped[key].breakdown[platformIndex].amount += item.amount;
      } else {
        grouped[key].breakdown.push({ platform: item.platform, amount: item.amount });
      }
    });
  ...
  return (
    <div className="space-y-10">
      ...
      {user.role === 'artist' && (
        <>
          {/* Balance Card */}
          <div className="vibrant-gradient p-8 md:p-12 rounded-[2.5rem] md:rounded-[3rem] text-white shadow-2xl shadow-brand-primary/30 relative overflow-hidden group">
            <div className="relative z-10">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-60 mb-2">Available Balance</p>
              <h2 className="text-4xl md:text-7xl font-black tracking-tighter mb-10">${balance.toFixed(2)}</h2>
              <div className="flex flex-wrap gap-4">
                <button 
                  onClick={() => setShowWithdraw(true)}
                  className="flex-1 md:flex-none bg-white text-black px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-xl"
                >
                  Withdraw Funds
                </button>
                <button 
                  onClick={() => setShowStatements(true)}
                  className="flex-1 md:flex-none bg-white/20 backdrop-blur-md text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-white/30 transition-all"
                >
                  View Statements
                </button>
              </div>
            </div>
            <Wallet className="absolute -bottom-10 -right-10 w-48 h-48 md:w-64 md:h-64 text-white/10 rotate-12 group-hover:rotate-0 transition-transform duration-1000" />
          </div>

          {/* Payout Information */}
          <div className="glass-card p-6 md:p-10">
            <div className="flex items-center gap-4 mb-8">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-brand-primary/10 flex items-center justify-center text-brand-primary shadow-lg">
                <CreditCard className="w-5 h-5 md:w-6 md:h-6" />
              </div>
              <div>
                <h3 className="text-lg md:text-xl font-black text-white tracking-tight">Payout Method</h3>
                <p className="text-white/40 text-[10px] font-black uppercase tracking-widest mt-1">Where your money goes</p>
              </div>
            </div>
            <div className="p-4 md:p-6 bg-white/5 rounded-2xl md:rounded-3xl border border-white/5 flex items-center justify-between group cursor-pointer hover:bg-white/10 transition-all">
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="font-black text-white text-sm tracking-tight truncate">Bank Transfer (SWIFT/SEPA)</p>
                  <p className="text-[10px] font-black text-white/20 uppercase tracking-widest mt-0.5 truncate">{user.bank_info || 'Primary Method'}</p>
                </div>
              </div>
              <button 
                onClick={() => setShowPayoutEdit(true)}
                className="text-[10px] font-black text-brand-primary uppercase tracking-widest hover:text-brand-secondary transition-colors shrink-0"
              >
                Edit
              </button>
            </div>
      ...
      <div className="glass-card overflow-hidden">
        ...
        <div className="overflow-x-auto scrollbar-hide">
          ...
          <div className="md:hidden divide-y divide-white/5">
            {user.role === 'admin' ? (
              withdrawals.length === 0 ? (
                <div className="px-8 py-20 text-center text-white/20 italic font-medium">No withdrawal requests</div>
              ) : (
                withdrawals
                  .filter(w => activeTab === 'all' || activeTab === 'payouts')
                  .map(w => (
                    <div key={w.id} className="p-8 hover:bg-white/5 transition-all space-y-6" onClick={() => setSelectedWithdrawal(w)}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-black text-white tracking-tight">{w.artist_name}</p>
                            <p className="text-[10px] font-black text-white/20 uppercase tracking-widest">{new Date(w.created_at).toLocaleDateString()}</p>
                          </div>
                        </div>
                      ...
                      <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                        <p className="text-[8px] font-black text-white/20 uppercase tracking-widest mb-1">Bank Information</p>
                        <p className="text-[10px] font-bold text-white/60 leading-relaxed">{w.bank_info || 'Not provided'}</p>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-3" onClick={e => e.stopPropagation()}>
                        {w.status === 'pending' ? (
                          <>
                            <button 
                              onClick={() => handleAdminAction(w.id, 'approved')}
                              className="flex-1 px-4 py-3 bg-emerald-500/10 text-emerald-400 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-emerald-500/20 transition-all border border-emerald-500/20"
                            >
                              Approve
                            </button>
                            <button 
                              onClick={() => handleAdminAction(w.id, 'rejected')}
                              className="flex-1 px-4 py-3 bg-rose-500/10 text-rose-400 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-rose-500/20 transition-all border border-rose-500/20"
                            >
                              Reject
                            </button>
                          </>
                        ) : w.status === 'approved' ? (
                          <label className="w-full px-4 py-3 bg-brand-primary/10 text-brand-primary rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-brand-primary/20 transition-all cursor-pointer flex items-center justify-center gap-2 border border-brand-primary/20">
                            <Upload className="w-3 h-3" /> Mark Transferred
                            <input 
                              type="file" 
                              className="hidden" 
              ...
              (activeTab === 'all' 
                ...
                .sort((a: any, b: any) => new Date(b.created_at || b.date).getTime() - new Date(a.created_at || a.date).getTime())
                .map((tx: any) => (
                  <div key={tx.id + tx.type} className="p-8 hover:bg-white/5 transition-all flex items-center justify-between gap-4" onClick={() => tx.type === 'withdrawal' && setSelectedWithdrawal(tx)}>
                    <div className="flex items-center gap-4 min-w-0">
                      <div className={cn(
                      ...
                      <div className="min-w-0">
                        <p className="font-black text-white tracking-tight truncate">{tx.description || (tx.type === 'revenue' ? 'Streaming Revenue' : 'Withdrawal Request')}</p>
                        <p className="text-[10px] font-black text-white/20 uppercase tracking-widest">{new Date(tx.created_at || tx.date).toLocaleDateString()}</p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {statements.length === 0 ? (
                      <div className="col-span-full py-20 text-center">
                        <p className="text-white/20 font-black text-[10px] uppercase tracking-widest">No statements found for this period</p>
                      </div>
                    ) : (
                      statements.map((stmt) => (
                        <div 
                        ...
                        >
                          <div className="flex items-center gap-6">
                            ...
                            <div>
                              <p className="font-black text-white tracking-tight">{stmt.title}</p>
                              <p className="text-[10px] font-black text-white/20 uppercase tracking-widest mt-1">{stmt.period}</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="font-black text-white tracking-tight">${stmt.amount.toFixed(2)}</p>
                            <ChevronRight className="w-5 h-5 text-white/10 group-hover:text-white transition-all ml-auto mt-1" />
                          </div>
                        </div>
                      ))
                    )}
