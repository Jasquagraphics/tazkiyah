  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setRecentReleases(Array.isArray(releases) ? releases.slice(0, 3) : []);
        ...
        setStats({
          ...
          activeReleases: Array.isArray(releases) ? releases.length : 0,
          ...
          <div className="flex items-center justify-between mb-10">
            ...
            <div className="flex gap-2 bg-white/5 p-1 rounded-2xl">
              <button 
                ...
                className={cn("px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all", (stats.filter === '7' || !stats.filter) ? "bg-white/10 text-white" : "text-white/40 hover:text-white")}
              >
                7 Days
              </button>
              <button 
                onClick={() => setStats({...stats, filter: '30'})}
                className={cn("px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all", stats.filter === '30' ? "bg-white/10 text-white" : "text-white/40 hover:text-white")}
              >
                30 Days
              </button>
            </div>
